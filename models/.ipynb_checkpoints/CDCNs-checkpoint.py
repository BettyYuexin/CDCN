...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        # 最大池化和均值的concatenation进行卷积
        # refine and fuse low-mid-high levels CDC features via spatial attention
...
# 输入[3, 256, 256]的RGB的脸的图像，输出通过multi-level fused features得到的grayscale的
# facial depth [1, 32, 32]
class CDCNpp(nn.Module):
    ...
    def forward(self, x):	    	# x [3, 256, 256]
        ...
        x_input = x
        x = self.conv1(x)		    # [128, 256, 256]   
        # attention的部分对应了MAFM
        x_Block1 = self.Block1(x)	   # [128, 128, 128] 	    	
        attention1 = self.sa1(x_Block1)     # [1, 128, 128]
        # 对应项相乘 [128, 128, 128] (output[i][j][k] = x_Block1[i][j][k] * attention[0][j][k])
        x_Block1_SA = attention1 * x_Block1 
        x_Block1_32x32 = self.downsample32x32(x_Block1_SA)   # [128, 32, 32]
        
        x_Block2 = self.Block2(x_Block1)	# [128, 64, 64]    
        attention2 = self.sa2(x_Block2)  
        x_Block2_SA = attention2 * x_Block2
        x_Block2_32x32 = self.downsample32x32(x_Block2_SA)  # [128, 32, 32] 
        
        x_Block3 = self.Block3(x_Block2)	# [128, 32, 32]    
        attention3 = self.sa3(x_Block3)  
        x_Block3_SA = attention3 * x_Block3	
        x_Block3_32x32 = self.downsample32x32(x_Block3_SA)   # [128, 32, 32]
        
        # [128*3, 32, 32], dim=1因为前面还有batch的维度
